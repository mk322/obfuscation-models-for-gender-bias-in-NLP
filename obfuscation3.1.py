...
for i, v in male_text.items():
    ...
    words = v.split()
    for word in words:
        if word in male_words and random.choice([True, False]):
            if male_words[word] != 'VOO':
...
for i, v in female_text.items():
    ...
    words = v.split()
    for word in words:
        if word in female_words and random.choice([True, False]):
            if female_words[word] != 'VOO':
...
dataset_c.to_csv(r'step3.1.csv', index = False)
print('Finished')