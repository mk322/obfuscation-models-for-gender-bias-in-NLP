...
for i, v in male_text.items():
    ...
    for word in words:
        if word in male_words:
            if male_words[word] != 'VOO' and male_words[word] != 'NCW':
...
for i, v in female_text.items():
    ...
    for word in words:
        if word in female_words:
            if female_words[word] != 'VOO' and female_words[word] != 'NCW':
...
dataset_c.to_csv(r'step3.3.csv', index = False)
print('Finished')